...
void handle_pipeline() {
    ...
    /* clear bubble after applying it */
    bubble = false;
}
...
void EX() {
    ...
    if (opcode == BRANCH_OPCODE) {
        /* Compute branch condition and target */
        uint32_t target = ID_EX.PC + ID_EX.imm;
        bool take = false;
        uint8_t f3 = GET_FUNCT3(ID_EX.IR);
        switch (f3) {
            case 0x0: take = (ID_EX.A == ID_EX.B); break; // BEQ
            case 0x1: take = (ID_EX.A != ID_EX.B); break; // BNE
            case 0x4: take = ((int32_t)ID_EX.A <  (int32_t)ID_EX.B); break; // BLT
            case 0x5: take = ((int32_t)ID_EX.A >= (int32_t)ID_EX.B); break; // BGE
            case 0x6: take = (ID_EX.A <  ID_EX.B); break; // BLTU
            case 0x7: take = (ID_EX.A >= ID_EX.B); break; // BGEU
        }
        if (take) {
            NEXT_STATE.PC = target & ~0x3;
			/* Stall IF and flush the fetched instruction */
			bubble     = true;
        	IF_ID.IR   = 0;
        }
    }
        ...
        uint8_t rd = (ID_EX.IR >> 7) & BIT_MASK_5;
        NEXT_STATE.REGS[rd] = ID_EX.PC + 4;
        /* Compute jump target and update PC */
        NEXT_STATE.PC = (ID_EX.PC + ID_EX.imm) & ~0x3;
        /* Stall IF and flush following instruction */
        bubble = true;
        IF_ID.IR = 0;
    }
    ...
    if (ENABLE_FORWARDING) {
        ...
        /* forward from MEM/WB */
        {
            ...
            if (mem_wb && mem_rd != 0) {
                uint32_t rs = (ID_EX.IR >> 15) & BIT_MASK_5;
                uint32_t rt = (ID_EX.IR >> 20) & BIT_MASK_5;
                uint32_t val = (mem_op == LOAD_OPCODE ? MEM_WB.LMD : MEM_WB.ALUOutput);
...
void ID() {
    ...
    /* —— Hazard Detection —— */
    {
        ...
        /* If forwarding OFF: stall on any RAW hazard.
         * If forwarding ON: only stall on load-use hazard. */
        if ((!ENABLE_FORWARDING &&
            ...
            /* insert one bubble: freeze IF, flush this ID/EX */
            bubble = true;
            ID_EX.IR  = 0;
            ID_EX.A   = 0;
            ID_EX.B   = 0;
    ...
    /* Record PC of this instruction for branch/jump */
    ID_EX.PC  = IF_ID.PC - 4;
    ID_EX.IR  = IF_ID.IR;
    ID_EX.A   = regA;
    ID_EX.B   = regB;
...
/************************************************************/
void decode(uint32_t * opcode, uint32_t * f3, uint32_t * f7, uint32_t * rd,
    ...
    *opcode = GET_OPCODE(IF_ID.IR);
    ...
    switch (*opcode) {
        ...
        case STORE_OPCODE: {
            uint32_t imm4_0  = (IF_ID.IR >> 7)  & BIT_MASK_5;
            uint32_t imm11_5 = (IF_ID.IR >> 25) & BIT_MASK_7;
            *f3   = (IF_ID.IR >> 12) & BIT_MASK_3;
            *rs1  = (IF_ID.IR >> 15) & BIT_MASK_5;
            *rs2  = (IF_ID.IR >> 20) & BIT_MASK_5;
        ...
        case BRANCH_OPCODE: {
            ...
            *rs2 = (IF_ID.IR >> 20) & BIT_MASK_5;
            ...
            uint32_t val = (imm12 << 12) | (imm11 << 11)
                         | (imm10_5 << 5) | (imm4_1 << 1);
            if (val & (1 << 12)) val |= ~((1 << 13) - 1);
            *imm = val;
            break;
        }
        ...
        case JUMP_OPCODE: {
            *rd  = (IF_ID.IR >> 7) & BIT_MASK_5;
            ...
            uint32_t val = (imm20 << 20) | (imm19_12 << 12)
                         | (imm11 << 11) | (imm10_1 << 1);
            if (val & (1 << 20)) val |= ~((1 << 21) - 1);
            *imm = val;
            break;
        }
...
/************************************************************/
/* Print the program loaded into memory (in RISCV assembly)  */
/************************************************************/
void print_program(){
    for (uint32_t addr = MEM_TEXT_BEGIN;
         addr < MEM_TEXT_BEGIN + PROGRAM_SIZE * 4;
         addr += 4) {
        uint32_t cmd = mem_read_32(addr);
        print_command(cmd);
        printf("\n");
    }
}
...
/************************************************************/
/* Print a single instruction in assembly form               */
/************************************************************/
